...
local function get_targets(winid, char, direction, test_func)
  -- Case sensitive pattern:
  -- local match_regex = "\\v[[="..char.."=]]\\C"
  -- Ignore case pattern:
  local match_regex = "\\v[[="..char.."=]]\\c" -- no escaping is fine ?  -- doesn't work: '\\v(%d'..tostring(vim.fn.char2nr(char))..')\\c'
  ...
  while lnum <= finish do
    ...
    if fold_end ~= -1 then
      ...
      for i, cur in ipairs(chars) do -- search beyond last column
        ...
        if test(cur, match_regex) and test_func(chars, i) then
...
local function leap(opts, leap_override_opts)
  ...
  local test_func
  do
    local opt_type = type(opts.test_function)
    if not opts.test_function then test_func = test_regular
    elseif opt_type == "function" then test_func = opts.test_function
    elseif opt_type == "string" then
      if opts.test_function == "regular" then test_func = test_regular
      elseif opts.test_function == "split_identifiers" then test_func = test_split_identifiers
      else print("no test function named `" .. opts.test_function .. '`') end
    else print("Invalid value for `test_function`") end
  end
  if test_func == nil then return end
